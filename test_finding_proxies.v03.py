...
def test_all_cpds_all_finding_proxies(json_dump_dir, grouping_type, error_log_filepath, test_all_cpds_except_these = None):
	...
	try:
		with open(error_log_filepath, 'x') as test_obj:
			print('', file=test_obj)
			
	except FileExistsError:
		print("The log file already exists.  Please call me again with a different log file")
...
def test_finding_proxy(fnd_proxy_json, consult_json, grouping_type, error_log_filepath):
	...
	potential_error_msg_txt_4grep = f' CPD | {consult_id} | PRN |  {program_rec_num} | "{program_title}" | "{consult_name}" | CFNDS '
...
The context in which the error was obtained: {error_report_obj}
...
def ret_all_combinations_unless_too_many(mylist, max_number_of_combos_to_return):
	...
	>>> ret_all_combinations_unless_too_many(['a','b','c'], 3)
	False
