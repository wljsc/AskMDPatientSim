...
def post_next(payload, log_dir, question_index_seq):
	...
	if error_msg:
		...
		print(error_msg)
		print()
		print("TODO TODO TODO don't forget to log error in logs!!")
		pdb.set_trace()
...
def process_system_reply(json_input, log_dir):
	...
	for resp_section_num, response_section in enumerate(response_sections):
		...
		print()
		print(f"SYSTEM QUESTION  {question_index_seq}:", response_header)
		log(conversation_log_filepath, f"* SYSTEM QUESTION  {question_index_seq}: {response_header}")
		print()		
		...
		for possible_user_response in enumerate(allowed_responses):
			...
			log(conversation_log_filepath, f"{response_number}: {response_txt}")
