...
def test_guidance_options_full_scale(dir_of_dir_of_dirs, verbose_p = True):
	...
	for run_group_dir in list_of_run_group_dirs:
	    test_guidance_options_from_many_dirs(run_group_dir, base_dir = dir_of_dir_of_dirs)

	print(f"{utils.now_yyyy_mm_ddThhmmss()} Completed @fun test_guidance_options_from_scale_up on {num_run_of_groups} run group directories")
	print("################################################################")
	print("################################################################")

def test_guidance_options_from_many_dirs(dir_of_dirs, base_dir = None, verbose_p = True):
	...
	print("################################################################")
	print("################################################################")
	print(f"{utils.now_yyyy_mm_ddThhmmss()} Started test_guidance_options_from_many_dirs on {num_questionaire_result_dirs} questionnaire result dirs")
	print()
	...
	################ Finally, We have a Case of 'Success' so error_report_string should be the empty string
		...
		if verbose_p:
			...
			if PRINT_MASSIVE_SPEW_P:
				...
				print("PRINT_MASSIVE_SPEW_P is false so we aren't going to print the Finding List Post")
			print()
