...
def post_findings_to_guidance_options(payload_as_json_str, log_dir = None):
	...
	################################################################
	# SECTION BLINDLY COPIED FROM the definition of @func question_and_answer_loop in test_question_driver.py needing modification (START OF SECTION)
	
	if log_basedir is not None:
		if timestamp_in_log_dir_p:
			sub_dir_for_this_run = f"{LOG_DIR_PREFIX}.{utils.now_yyyy_mm_ddThhmm()}-Cpd-{consultation_id}"
		else:
			sub_dir_for_this_run = f"{LOG_DIR_PREFIX}-Cpd-{consultation_id}"
	...
	# SECTION BLINDLY COPIED FROM the definition of @func question_and_answer_loop in test_question_driver.py needing modification (EDN OF SECTION)
	################################################################
