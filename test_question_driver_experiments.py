...
# 2022-10-21T07:10:12PDT back after a long hiatus.  saved latest
# version as test_question_driver_experiments.2022-10-09T1407.py
...
from test_question_driver import now_yyyy_mm_ddThhmm
...
# Weeks later at 2022-10-21T07:32:50PDT trying to dust the above off bc

# for consult_number in DOB_First_Height_Weight_Then_All_Zero_Unless_Error_Yes_Working:
#     question_and_answer_loop(consult_number,
# 	log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-10-21T0721.FirstHeightWeight-ThenAllZero/",
# 	patient_sim_script_fpath = "../PatientSimulatorScripts/dob_height_weight_then_all_zero_unless_error.json")

# At 2022-10-22T10:18:04PDT I *think* the above was aborted for some
# reason. Maybe bc of the bug that I reported in that Friday morning.
# Michael fixed it later in the morning.  I was feeling confident so I
# decided nah I don't need todo the 'safe' thing of merely repeating
# what worked before.  Let me move on to something a little riskier.

################################################################
# 2022-10-21T07:11:39PDT trying to do an analogous repeat of the above.
#
# in the above I used...
# 
# DOB_First_Height_Weight_Then_All_Zero_Unless_Error_Yes_Working
...
# for consult_number in DOB_First_Height_Weight_Then_All_Zero_Unless_Error_Yes_Working:
#     question_and_answer_loop(consult_number,
# 	log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-10-21.SexDOBHeightWeight/",
# 	patient_sim_script_fpath = "../PatientSimulatorScripts/dob_height_weight_then_all_zero_unless_error.json")

# The above did not work.
# I got this error...
# NameError: name 'ASK_OPERATOR_HOW_TO_HANDLE_BOT_ERROR_STR' is not defined
# it seemed to start with CPD # 70 as it should, but then
# started it again, why?  and then it threw that error.

# 2022-10-22T10:35:24PDT NOW I see an almost comedy of errors.  I
# *said* I was going to use the new thing,
# i.e. RemainingConsultsAssumed_SexDOBHeightWeight_yes_okay but in
# fact I did not.  Instead, I used the old
# DOB_First_Height_Weight_Then_All_Zero_Unless_Error_Yes_Working.

################################################################

# 2022-10-22T10:15:33PDT started off on the wrong track
# did a test run.
# realized it was erroneous.
# 2022-10-22T10:38:43PDT better oriented.

# So, we know from the above that...
# DOB_First_Height_Weight_Then_All_Zero_Unless_Error_Yes_Working = \
# [70, 72, 85, 89, 131, 146, 188, 189]
# ...and with 70 we are getting...
#
# NameError: name 'ASK_OPERATOR_HOW_TO_HANDLE_BOT_ERROR_STR' is not defined
#
# ...let's see if we get it with 72

# for consult_number in [72, 85, 89, 131, 146, 188, 189]:
#     question_and_answer_loop(consult_number,
# 	log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-10-22T1041.DebuggingTest/",
# 	patient_sim_script_fpath = "../PatientSimulatorScripts/dob_height_weight_then_all_zero_unless_error.json")

################################################################
# 2022-10-24T11:19:08PDT trying to see if i can replicate this'get to last question and then start from the beggining behavior"

# question_and_answer_loop(85, log_basedir = "../Logs-Generic/MiscTests/")

################################################################
# 2022-10-27T16:32:20PDT

# michael said he fixed it the oher day

# for consult_number in [72, 85, 89, 131, 146, 188, 189]:
#      question_and_answer_loop(consult_number,
#  	log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-10-27T1632.DebuggingTest/",
#  	patient_sim_script_fpath = "../PatientSimulatorScripts/dob_height_weight_then_all_zero_unless_error.json")
# 2022-10-28T12:32:26PDT

################################################################
# 2022-10-28T11:05:07PDT Thinkgs seem to work

# 2022-10-28T1103

# for consult_number in DOB_First_Height_Weight_Then_All_Zero_Unless_Error_Yes_Working:
#     question_and_answer_loop(consult_number,
# 	log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-10-28T1103.SexDOBHeightWeight-ThenAllZero",
# 	patient_sim_script_fpath = "../PatientSimulatorScripts/dob_height_weight_then_all_zero_unless_error.json")

# then 

# but then reazied put in wrong name dir

# bc used DOB_First_Height_Weight_Then_All_Zero_Unless_Error_Yes_Working:

# thus did this move....

# (env) william.jarrold@bactespc6217 GenDataForTestGuidOpt % mv 2022-10-28T1103.SexDOBHeightWeight-ThenAllZero/ 2022-10-28T1103.DOBFirstHeightWeightThenAllZeroUnlessError
# <T1103.SexDOBHeightWeight-ThenAllZero/ 2022-10-28T1103.DOBFirstHeightWeightThenAllZeroUnlessError
# 2022-10-28T1103.SexDOBHeightWeight-ThenAllZero/ -> 2022-10-28T1103.DOBFirstHeightWeightThenAllZeroUnlessError

################################################################
# 2022-10-28T12:25:53PDT now gonna really try to do a large bunch

# for consult_number in RemainingConsultsAssumed_SexDOBHeightWeight_yes_okay:
#      question_and_answer_loop(consult_number,
#  	log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-10-28T1230.SexDOBHeightWeight",
#  	patient_sim_script_fpath = "../PatientSimulatorScripts/sex_dob_height_weight_then_all_zero_unless_error.json"
# )

################
# 2022-10-30T17:12:17PDT Noticed that the above threw an error at 182 See [[file:~/ShCr/AskMD/Resources/Couplet/TestCoupletPrograms/diary-TestCoupletPrograms.org::Link-2022-10-30-Sun-17-20][this]]

# /Users/william.jarrold/ShCr/AskMD/Resources/Couplet/TestCoupletPrograms/Logs-Generic/QuestionDriverTests/2022-10-30-DiagnoseIssueWithCPD-182

# So, let's see if we can repet it:

# for consult_number in [182]:
#      question_and_answer_loop(consult_number,
#  	log_basedir = "../Logs-Generic/QuestionDriverTests/2022-10-30-DiagnoseIssueWithCPD-182/",
#  	patient_sim_script_fpath = "../PatientSimulatorScripts/sex_dob_height_weight_then_all_zero_unless_error.json"
# )

# 2022-10-30T17:22:29PDT yes I repeated it.

################
# 2022-11-08T10:32:49PST the next large bunch


#BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay = \
# SCRIPT: sex_dob_height_weight_then_all_zero_unless_error.json

# for consult_number in BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay:
#      question_and_answer_loop(consult_number,
#  	log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-11-08T1034.BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay",
#  	patient_sim_script_fpath = "../PatientSimulatorScripts/sex_dob_height_weight_then_all_zero_unless_error.json"
# )



# BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay.remove(3)
# BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay.remove(6)
# BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay.remove(7)
# for consult_number in BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay:
#      question_and_answer_loop(consult_number,
#  	log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-11-08T1034.BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay",
#  	patient_sim_script_fpath = "../PatientSimulatorScripts/sex_dob_height_weight_then_all_zero_unless_error.json"
...
# 2022-11-08T19:43:38PST it errored on 29
# 
# this is what it has...
# BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay = \
# [3, 6, 7, 8, 9, 11, 14, 15, 16, 17, 20, 24, 27, 29, 32,
# 40, 47, 49, 55, 57, 59, 66]
# so this is what is remaining:
# 32, 40, 47, 49, 55, 57, 59, 66

# for consult_number in [29, 32, 40, 47, 49, 55, 57, 59, 66]:
# 	this_runs_timestamp = now_yyyy_mm_ddThhmm()
# 	this_runs_log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-11-08T1034.BirthSexFirst_Height_Weight_AllZeroUnlessError_Okay"
# 	try:
	
# 		question_and_answer_loop(consult_number,
# 			log_basedir = this_runs_log_basedir,
# 			patient_sim_script_fpath = "../PatientSimulatorScripts/sex_dob_height_weight_then_all_zero_unless_error.json",
# 			timestamp_in_log_dir = this_runs_timestamp)
# 	except Exception as error_msg:
# 		print("################################################################")
# 		print("#### Exception Thrown !!!!! ####################################")
# 		print(f"#### Consult {consult_number} #################################")
# 		print(f"#### error_msg is {error_msgs} #################################")
# 		print("################################################################")
# 		filename_of_error_file = "Error." + sub_dir_name_for_consult_test_run(this_runs_timestamp, consult_number) + ".txt"
# 		filepath_of_error_file = os.path.join(this_runs_log_basedir, filename_of_error_file)
...
for consult_number in ChildrensConsults_not_suffering_Consys_322:
	this_runs_timestamp = now_yyyy_mm_ddThhmm()
	this_runs_log_basedir = "../Logs-Generic/GenDataForTestGuidOpt/2022-11-08T2242.ChildrensConsults_not_suffering_Consys_322"
	try:
