...
################
# test_finding_proxies.2022-09-05T1225.py

# doing some very deliate confusing changes to error / content reporting

################
# test_finding_proxies.2022-09-05T102028.py

# we are partway through making a number of changes all catalyzed by
# the problem of the program halting when we get a non-standard
# status_code, e.g. post_response.status_code: 403

# We are refactoring error handling.  Realizing three kinds of errors.
# (1) Throw exception while calling endpoint
# (2) Non-success status code frrom endpoint
# (3) error from within endpoint.

################
# 2022-09-02T08:40:18PDT reverted to prior version (temp gave up on output to log file)
# 
# (env) william.jarrold@SC7618 Code % ll *.py
# ll *.py
# <<<< ..... snip ..... >>>
# -rw-r--r--  1 william.jarrold  staff   40532 Jul 21 09:43 test_finding_proxies.2022-07-21T0943.py
# -rw-r--r--  1 william.jarrold  staff   40683 Aug 16 07:42 test_finding_proxies.v05.py
# -rw-r--r--  1 william.jarrold  staff   14074 Aug 31 16:36 test_finding_proxies_experiments.py
# -rw-r--r--  1 william.jarrold  staff   44474 Sep  2 06:56 test_finding_proxies.2022-09-02T0656.py
# -rw-r--r--  1 william.jarrold  staff   46306 Sep  2 08:37 test_finding_proxies.2022-09-02T0713.partial_attempt_to_save_to_log_file.py
#
# test_finding_proxies.2022-09-02T0713.partial_attempt_to_save_to_log_file.py
# <<< ------- this is the version that i started to save to log file rather than stdio but then i gave up temporarility
#
#
# test_finding_proxies.2022-09-02T0656.py
# <<< ------- this is the 'prior version' (has not log to file, only log to stdio) .. we reverted to that
#
#
#
# (env) william.jarrold@SC7618 Code % cp test_finding_proxies.2022-09-02T0656.py test_finding_proxies.py
# cp test_finding_proxies.2022-09-02T0656.py test_finding_proxies.py
# test_finding_proxies.2022-09-02T0656.py -> test_finding_proxies.py
################
################
# 2022-07 around the time this file was first created
...
ErrType_PostExc_T1 = "ExceptionDuring1stPost"
ErrType_PostExc_T2 = "ExceptionDuring2ndPost"

# ErrType_StatusNotSuccess = "StatusNotSuccess"
# StatusNotSuccess means status_code is not 200.  If we get one of those
# we should try again.  Thus, push it into the logic for trying again
# and it's siplmetd to just call this case ExceptionDuring1stPost or 2nd
# Thus as of 2022-09-05T13:04:26PDT I've decided , let's NOT implement ErrType_StatusNotSuccess


ErrType_ReplyIsError = "ReplyMsgContainsError"
...
def test_all_cpds_all_finding_proxies(json_dump_dir, grouping_type, error_log_filepath, test_all_cpds_except_these = None):
	...
	for cpd_num in cpd_nums:
		...
		print(f"#### Duration of EntireConsult: {duration_for_this_cpd_in_secs} (secs) Con#: {cpd_file_base_name} {utils.now_yyyy_mm_ddThhmmss()}", test_output_fobj)
		print("################################################################", test_output_fobj)
...
def test_finding_proxies_in_cpd_file(cpd_filepath, grouping_type, error_log_filepath):
	...
	with open(cpd_filepath, 'r') as fobj:
		...
		print("################################################################")
		print(f"For Consult# {consult_id}, there are {len(proxy_rec_nums_by_cntrbg_fnd_rec_nums)} programs to be tested")
		print("################################################################")		
		...
		for proxy_record_number in proxy_rec_nums_by_cntrbg_fnd_rec_nums:
			...
			print(f"#### Duration WholeProxy: {duration_in_secs} (secs) of Cpd {consult_id} ProgRecNum: {proxy_record_number}")
			print("################################################################")

			

def test_finding_proxy(fnd_proxy_json, consult_json, grouping_type, error_log_filepath, validation_mode = True, if_post_exception_log_retry_and_continue_p = True):
	"""
	...
	For each grouping of contributing findings, it calls the
	finding proxy (a program).  The goal is to as exhaustively as
	possible then the finding proxy.

	The simplest level of testing is just to detect and
	appropriately log errors that occur when the given finding
	proxy is run with some set of contributing findings set.  

	A higher level of testing is to not only make sure that no
	errors occur but also to ensure that the result of the finding
	proxy program is correct (see @param validation_mode)

	Sometimes errors are thrown when calling the finding proxy
	endpoint on a given finding proxy / group of contributing
	findings.  If there is an error, this function will hit the
	endpoint one more time.  If it fails again, then it will move
	on (and run the next group of contributing findings).  (See
	@param if_post_exception_log_retry_and_continue_p)

	Searching the Logs for Errors:

	There are some useful tricks you can use if, after a run, you
	grep the contents of @param error_log_filepath (actually as of
	2022-09-05T09:56:52PDT we aren't sending output to that file.
	We are merely printing to standard output.  See files like
	detail-diary-TestCoupletPrograms-trace-of-test-finding-proxy-2022-08-22.org
	for cases in which such standard output trace has been saved
	to a file for more analysis / quantification etc.

	Searching for "4GREP", "exception", "error" in the output
	(whether it is standard out or in a log file) are ways you can
	use to uncover ways to semi-automatically find / track /
	quantify errors.  A TODO is to make this more uniform,
	probably make 4GREP the way to grep for any issue.

	@param grouping_type

	This parameter specifies how groupings of contributing
	...
	contributing findings).  This can be combinatorially
	explosive.  For example, some finding proxies have 20 or more
	contributing findings.  2^20 is just over 1 million.  Thus,
	we'd have to run the program 1M times to exhaustively test it
	against all combinations.  If it takes only 1 second to run
	the finding proxy on any given combination, 1M seconds
	converts to over 11 days!!

	How do we manage this explosiveness?

	Well, sometimes we don't have to be so exhaustive.  E.g.,
	Sometimes a grouping like 'just try each contributing finding
	on its own' is good enough for the current level of testing
	desired.  See @func compute_finding_groups for a definition of
	each kind of grouping_type.  See OneFindingPerGroup.
	...
	@param if_post_exception_log_retry_and_continue_p: at
	2022-09-04 I think this function makes no sense if the value
	of @param if_post_exception_log_retry_and_continue_p is not
	True.  See 2022-09-04T10:29:21PDT below.  Therefore this
	parameter should probably go away.
	...
	#
	# TOOD(2022-09-05T12:45:31PDT, ) This block (i.e. for
	# potential_error_msg_txt_4grep) has been obviated by the
	# function print_4_grep.  We keep it here only in case we need
	# to backout of any changes.  Thus, if this block is still
	# around like 2 days from now then blow away this block with
	# extreme prejudice bc any problems (assuming the refacotring
	# has been completed) was likely successful (assuming heavy
	# testing happens ver the next 2 days)
	
	# error_report_obj = [potential_error_msg_txt,
	# potential_error_msg_txt_4grep] TOOD(2022-09-05T12:45:31PDT,
	# ) the above line has been obviated by the function
	# print_4_grep.  We keep it here only in case we need to
	# backout of any changes.  Thus, if this block is still around
	# like 2 days from now then blow away this block with extreme
	# prejudice bc any problems (assuming the refacotring has been
	# completed) was likely successful (assuming heavy testing
	# happens ver the next 2 days)
	...
	for combo_of_fnd_ids in all_combos_of_contributing_finding_ids:
		...
		print("################################################################")
		print(f"#### {utils.now_yyyy_mm_ddThhmmss()} Cpd {consult_id} Prog {program_rec_num} Starting Attempt for Fnd Id Combo: {combo_of_fnd_ids}")
		print("################################################################")
		...
		# TOOD(2022-09-05T12:48:17PDT, ) This block (i.e. for
		# potential_error_msg_txt_4grep) has been obviated by the
		# function print_4_grep.  We keep it here only in case we need
		# to backout of any changes.  Thus, if this block is still
		# around like 2 days from now then blow away this block with
		...
		print(f"FirstTry: test_finding_proxy ({utils.now_yyyy_mm_ddThhmmss()}) combo_of_fnd_ids: {combo_of_fnd_ids} Cpd: {consult_id} RecNum: {program_rec_num}")

		bot_reply_to_post = {}

		# We need to set this in case we fail during the
		# 'Second Try' (see below).  If we haven't set this to
		# empty dict and we fail in 'Second Try' then we will
		# get...
		#
		#	UnboundLocalError: local variable 'bot_reply_to_post' referenced before assignment
		#
		# ...farther below.
		...
		# At 2022-09-04T10:29:21PDT I have come to believe the
		# this parameter solves not purpose.  I mean it if it
		# not True then the endpoint is not hit.  But the
		# whole point of this function (i.e. @func
		# test_finding_proxy) is to hit the enpoint.
		# Thus:
		#
		# (1) IF you agree with this paragraph
		# ...or...
		# (2) IF you do not understand this
		# paragraph...
		# THEN remove
		# @param if_post_exception_log_retry_and_continue_p from this
		# entire function.
		#
		# Else explain why it should remain and make any
		# assocaited code changes.
		
		exception_message_from_inside_except_clause = None
		
		# Why exception_message_from_inside_except_clause????
		# Well, I need to do this hackish thing because python
		# experiments show that when I catch an exception and
		# catch the message with the "as" part, e.g. as in
		#
		# <START QUOTE>
		# except Exception as my_exception_message
		# <END QUOTE>
		#
		# then whatever is bound to my_exception_message disappers, i.e.
		...
		if if_post_exception_log_retry_and_continue_p:
			...
			try:
			...
			except Exception as exception_message_1:
				exception_message_from_inside_except_clause = exception_message_1
				end_time_in_secs = round(time.time(), 2)
				duration_in_secs = round(end_time_in_secs - start_time_in_secs, 2)
				print(f"#### Duration of ProxyFindingCombination: {duration_in_secs} (secs) Con#: {consult_id}: Pro#: {program_rec_num} FndCombo: {combo_of_fnd_ids}  {utils.now_yyyy_mm_ddThhmmss()} (First Post Try Failed)")
				print("################################################################")
				print("# First Post Try Failed )-: )-: )-: ######################################")
				print("# Exception Message Text: (START OF SECTION) ###################")
				print(exception_message_1)
				print("# Exception Message Text: (END OF SECTION) ######################")
				print("################################################################")

				print_4_grep(consult_id, ErrType_PostExc_T1, exception_message_1, program_rec_num, program_title, consult_name, combo_of_fnd_ids)

				print("################################################################")
				print(f"SecondTry: test_finding_proxy ({utils.now_yyyy_mm_ddThhmmss()}) combo_of_fnd_ids: {combo_of_fnd_ids} Cpd: {consult_id} RecNum: {program_rec_num}")
				try:
				...
				except Exception as exception_message_2:
					exception_message_from_inside_except_clause = exception_message_1				
					end_time_in_secs = round(time.time(), 2)
					duration_in_secs = round(end_time_in_secs - start_time_in_secs, 2)
					
					print(f"#### Duration of ProxyFindingCombination: {duration_in_secs} (secs) Con#: {consult_id}: Pro#: {program_rec_num} FndCombo: {combo_of_fnd_ids}  {utils.now_yyyy_mm_ddThhmmss()} (Second Post Try Failed)")					
					print("################################################################")
					print("# Second Post Try Failed )-: )-: )-: ###########################")
					print("# Exception Message Text: (START OF SECTION) ###################")
					print(exception_message_2)
					print("# Exception Message Text: (END OF SECTION) ######################")
					print("################################################################")

					print_4_grep(consult_id, ErrType_PostExc_T2, exception_message_2, program_rec_num, program_title, consult_name, combo_of_fnd_ids)
					
				else:
					duration_in_secs = round(end_time_in_secs - start_time_in_secs, 2)
					print(f"#### Duration of ProxyFindingCombination: {duration_in_secs} (secs) Con#: {consult_id} Pro#: {program_rec_num} FndCombo: {combo_of_fnd_ids} {utils.now_yyyy_mm_ddThhmmss()} (Second Post Successful)")

			else:
				duration_in_secs = round(end_time_in_secs - start_time_in_secs, 2)
				print(f"#### Duration of ProxyFindingCombination: {duration_in_secs} (secs) Con#: {consult_id}: Pro#: {program_rec_num} FndCombo: {combo_of_fnd_ids} {utils.now_yyyy_mm_ddThhmmss()} (First Post Successful)")
		...
		if exception_message_from_inside_except_clause:
			print("################################################################")
			print(f"#### In Cpd {consult_id} Prog {program_rec_num} We've thrown two exceptions in a row working on this combo of finding ids {combo_of_fnd_ids}")
			print(f"#### the second exception message is: #########################")
			print(exception_message_from_inside_except_clause)
			print("##### Thus we are moving on to the next combination of finding ids")
			print("################################################################")			

		elif error_msg_in_endpoint_reply:
			print("error")
			...
			if fnds_added != []:
				print("Weird fnds_added is nonempty!")
				#pdb.set_trace()
			if fnds_removed != []:
		...
		elif validation_mode is None:
			print("Okay validation mode is None so we will not be doing any validation")

		else:
			print("How did you get here?  I thought all contingencies had been covered by the above if and elif's?")
			pbd.set_trace()

		print("################################################################")
		print(f"#### {utils.now_yyyy_mm_ddThhmmss()} Cpd {consult_id} Prog {program_rec_num} Completed Attempt for Fnd Id Combo: {combo_of_fnd_ids}")
		print("################################################################")
...
def post_contributing_findings(payload, error_report_obj, log_dir = None):
	...
	TODO(2022-09-05T12:14:02PDT, ) convert the below into a doctest?  Else why did I write it in this doc string?
	...
	print("################################################################")
	print("### Just Entered @fun post_contributing_findings ###############")
	print("################################################################")
	print()

	# At 2022-09-05T09:27:46PDT I replaced 'post_users_reply' with
	# 'post_contributing_findings' in the above print statement.
	# The comment you are reading right now is only important if
	# you are trying to sleuth logs that we made prior to this
	# time.  This must have been leftover detritus, i.e. the print
	# statement must not have been updated when the function name
	# was changed.
	...
	# TODO (2022-09-05T13:06:48PDT the next two ifs, i.e.
	#
	...
	#
	# I am tentatively keeping them around so that we can
	# understand old log files better - e.g. 'why are there no
	# indications of a status_code that is not 200'...  why?
	# because as you can see prior to 2022-09-05T13:06:48PDT this
	# would just throw an error right away there was no retry.
	...
	# 2022-09-05T12:38:11PDT The line above has been obivated by the call to print_4_grep
	# potential_error_msg_txt_4grep = error_report_obj[1]
	# we only retain this block of comments in case we need to back out.  So, if this block
	# is still here after say 2022-09-07 then delete this commented out block
	...
	print("################################################################")
	print("### About To Exit @fun post_contributing_findings ###############")
	print("################################################################")
...
################################################################
# Utility Misc Kinda Things

def print_4_grep(consult_id, error_type, error_msg_txt, program_rec_num, program_title, consult_name, combo_of_fnd_ids = None):

	if combo_of_fnd_ids is None:
		combo_of_fnd_ids_str = ""
	else:
	...
	| 4GREP | {error_type} | {error_msg_txt} | CPD | {consult_id} | PRN  | {program_rec_num} | {program_title} | {consult_name} | CFS | {combo_of_fnd_ids_str} | {utils.now_yyyy_mm_ddThhmmss()} |

	################################################################"""
	my_string = textwrap.dedent(for_grep_text)
	print(my_string)
...
# TODO(2022-09-05T12:06:41PDT, ) this commet text needs updating in light of message text changes
# DONE(2022-07-18T09:14:32PDT, 2022-09-05T12:06:27PDT make this into a function 
# currently the above 4GREP line looks like this...
# <START QUOTE>
