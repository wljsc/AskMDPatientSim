...
# test_question_driver.2023-02-12T1408.py
# right before making maybe big changes to allow for an auto-reply
# option to select all answers.

# 2023-02-04T09:41:57PST changing the format of UserReply

# test_question_driver.2023-01-29T1948.py
...
################################################################
# CONSTANTS used to be defined here but then most were imported

# TQD means Test Question Driver (as oppsed to Guidance Option Endpoint)
PRINT_MASSIVE_SPEW_OF_TQD_POST_P = False
PRINT_MASSIVE_SPEW_OF_TQD_RESPONSE_P = False
...
URL_AND_HEADERS_FILENAME = "url_and_headers_of_cons_srvc_post.json"

#DURATION_LOG_COLUMN_HEADER = ["consultation_id", "question_id", "duration_secs", "start_time_legible", "end_time_legible"]
# before 2023-02-12T11:26:01PST, after 2023-02-12T11:26:04PST added post_response code
DURATION_LOG_COLUMN_HEADER = ["consultation_id", "question_id", "duration_secs", "start_time_legible", "end_time_legible", "response_status_code"]
...
# 2023-02-11T23:56:39PST hacky attempt
# following https://stackoverflow.com/questions/15431044/can-i-set-max-retries-for-requests-request
...
# 2023-02-15T13:15:39PST change 10 to 1
retries = Retry(total=1,
                backoff_factor=1,
                status_forcelist=[ 500, 502, 503, 504 ],
		allowed_methods = frozenset({'POST', 'DELETE', 'GET', 'HEAD', 'OPTIONS', 'PUT', 'TRACE'}))

# 2023-02-12T11:55:19PST I added:
# allowed_methods = frozenset({'POST', 'DELETE', 'GET', 'HEAD', 'OPTIONS', 'PUT', 'TRACE'})
# beofre then it used the defqault which did not include POST and POST is what I do.
# TODO are there other non-200 statuses that we get?
SESSION.mount('http://', HTTPAdapter(max_retries=retries))

# https://urllib3.readthedocs.io/en/stable/reference/urllib3.util.html
# Seems to say the default are:
...
def question_and_answer_loop(
	...
	@param if_reply_script_UnlessError_means_LogAndContinue.

	If set to True, this absurdly named param says IF you are
	using a reply script (aka "auto mode") and you are in a
	situation where the bot returns a message saying something
	like 'i had some kind of error happen' THEN do not halt auto
	mode for a human to inspect.  No, march on.  Save stuff the
	same way you would if a hard exception would be thrown.  This
	is really to be used for scaling up stuff, i.e. see
	test_question_driver_scale_up.py (it is called in that file by
	@fun run_test_question_driver_over_consult_list)

	Of course, if set to False, then it will halt when it stumbles
	into an error and wait for human input.
	...
	################
	# 2023-02-07T18:39:28PST moved the manual mode/auto mode stuff to here.  Beofre it was
	# right at the start of this function, i.e. @fun question_and_answer_loop
	...
	if log_basedir is not None:
		...
		os.mkdir(log_dir)
		print()
		print(f"#### This conversation (driven by Cpd-{consultation_id} is being logged in {sub_dir_for_this_run} ################")
	...
	if log_dir is not None:
		...
		column_headers = DURATION_LOG_COLUMN_HEADER
		column_headers_str = f"{DURATION_LOG_SEP}".join(column_headers)
		print(column_headers_str, file=log_of_post_durations_fileobj)

		# Log How We Post: i.e. the URL and the headers
		
		#url_and_headers_of_post = {"url" : url, "headers" : POST_HEADERS}
		# 2023-02-09T13:42:16PST change url to hard call to config.QUESTION_DRIVER_ENDPOINT
		url_and_headers_of_post = {"url" : config.QUESTION_DRIVER_ENDPOINT, "headers" : POST_HEADERS}
	...
	while True:
		...
		################################################################
		# Three Big Branches Immediately Below

		# Big Branch 1: if we get error_msg back from bot

		# Big Branch 2: the bot tells us that we are done (bc
		# there is no question id) and so we wrap up the
		# conversation.

		# Big Branch 3: we get a 'normal' reply from the bot,
		# i.e. a new question or set of questions to answer.
		...
		if error_msg:
			...
			# TODO(2023-02-14T18:06:24PST, ) for readability this huge block should be put into it's own function
			...
			# In the below, we are receiving input from
			# the user in the context that an error has
			# just occurred, we are seeking input from the
			# user about what to do.  If we are in
			# auto-mode, then the user_reply_script_json
			# should tell us what to do.

			# To see the "normal" (i.e. non-error) code
			# for receiving user input see @fun
			# process_system_reply in
			# test_question_driver_process_system_reply.py
			...
			string_to_log_and_print = f"UserReply: '{choice_txt}' IsReplyToBotError for {sys_quest_num_str} in Cpd-{consultation_id} \n"			
...
def post_users_reply(payload,
	...
	MAIN PURPOSE: We post a payload (i.e. the users response to a
	question) to the bot.  That payload is, I *think* a json
	formatted string.  This function that returns what the bot
	'says' in response, and a couple of other details.
	...
	#!print("2023-02-12T12:31:34PST is log_of_post_durations_fileobj have a value?")
	#!pdb.set_trace()
	...
	# the below!!!!
	#
	# 2023-02-07T22:28:48PST remove this..
	#url = config.QUESTION_DRIVER_ENDPOINT
	#..to replace with hard call to config.QUESTION_DRIVER_ENDPOINT
	...
	if PRINT_MASSIVE_SPEW_OF_TQD_POST_P:
		print()
		print(json.dumps(payload_as_dict, indent = 4, sort_keys = True))		
		print()
	else:
		print("PRINT_MASSIVE_SPEW_OF_TQD_POST_P is false so we aren't going to print payload_as_json_str")			
	...
	# 2023-02-04T14:23:50PST change the above to the below to re-enable searching on 'error' to find problems
	# i.e. change "error" -> "(e r r o r)"
	MAX_RETRIES_EXCEEDED_WITH_URL_STR =  f"For sleep {sleep_duration} seconds - hoping to easily prevent 'Max_retries_exceeded_with_url' (e r r o r)"
	...
	if log_of_post_durations_fileobj:
		...
		# 2023-02-12T09:52:11PST replace time.time() with time.monotonic()
	start_time = time.monotonic()
	
	#post_response = requests.post(config.QUESTION_DRIVER_ENDPOINT, data=payload, headers=POST_HEADERS)
	# 2023-02-11T15:03:46PST trying this:
	#post_response = requests.post(config.QUESTION_DRIVER_ENDPOINT, data=payload, headers=POST_HEADERS, timeout = 1)
	# 2023-02-12T00:00:13PST trying this (the SESSION has the auto-retry thing)
	# post_response = SESSION.post(config.QUESTION_DRIVER_ENDPOINT, data=payload, headers=POST_HEADERS, timeout = 1)
	# 2023-02-12T10:35:17PST going back to no timeout, timeout was silly bc there was auto-retry of 10 with backoff_factor=1,
	post_response = SESSION.post(config.QUESTION_DRIVER_ENDPOINT, data=payload, headers=POST_HEADERS)
	...
	if log_of_post_durations_fileobj:
		...
		# 2023-02-12T14:08:03PST reviet to this revert to this maybe it will work again?
		row_values = [str(consultation_id), str(question_id), duration_secs, start_time_legible, end_time_legible]
		# 2023-02-12T11:27:54PST replace the above with the below (i.e. add status code)
		#values = [str(consultation_id), str(question_id), duration_secs, start_time_legible, end_time_legible, str(post_response.status_code)]

		#!print("2023-02-12T12:14:02PST why nothing being logged?")
		#!pdb.set_trace()
		#row_values = [str(consultation_id), str(question_id), duration_secs, start_time_legible, end_time_legible, str(post_response.status_code)]
		
		row_of_data_as_str = f"{DURATION_LOG_SEP}".join(row_values)
...
################################################################"""
	...
	# 2023-02-12T11:03:28PST golly why did not i go this earlier!!!!
	print("################################################################")
	print("################ post_response.status_code ################")
	print(f"################ post_response.status_code {post_response.status_code} ################")
	log_and_print_str_about_status_code = f"post_response.status_code: {post_response.status_code} : {sys_quest_num_str}"
	print(log_and_print_str_about_status_code)
	utils.log(conversation_log_filepath, log_and_print_str_about_status_code, print_to_stdout_too_p = True)
	print("################################################################")
	...
	if PRINT_MASSIVE_SPEW_OF_TQD_RESPONSE_P:
		print()
		print(pretty_bot_output_json_str)
		print()
	else:
		print("PRINT_MASSIVE_SPEW_OF_TQD_RESPONSE_P is false so we aren't going to print payload_as_json_str")			
...
# 2023-02-14T18:52:13PST hoping to verify that it can take multiple answers.
# question_and_answer_loop(
# 	start_conversation_with = 308,
# 	log_basedir = os.path.join(config.LOG_DIR_GENERIC, "QuestionDriverTests"),
# 	timestamp_in_log_dir = True)
