...
# (or even auto_mode?)  operator of the AskMD Patient Simulator
...
#
# But here is an attempt to summarize this.  Sometimes we want to compare to conversation files (i.e. 
...
def process_system_reply(
	...
	for resp_section_num, response_section in enumerate(response_sections):
		...
		if len(response_content_list) == 0:
			...
			# TODO(2022-06-23T17:03:33PDT, ) soon make this set_trace parameterized
			pdb.set_trace()
...
ResponseHeader: '{response_header}"""
		...
		# 2022-05-31T14:57:27PDT more printing changes, hopw they are okay (END OF SECTION)
				...
				# text_description_of_systems_typein_turn = f"Bot TypeIn Resp# {response_number}: {response_name}"
				# 2022-05-31T12:46:40PDT renaming Resp to BotSubQuestion# {response_number} (TypeIn)
				# text_description_of_systems_typein_turn = f'BotSubQuestion# {response_number}: SubQuestionTxt: "{response_name}" AnswerFormat: TypeIn'
...
def user_input_wrapper(prompt_text, user_reply_script_json = None, record_user_replies_for_replay_filepath = None):
...
def pretty_print_response_content_type_list(response_content_list):
	...
	print("response_content_type_abbrev_list:", response_content_type_abbrev_list)
