...
def question_and_answer_loop(start_conversation_with, log_basedir = None):
	...
	while True:
		...
		if error_msg:
			...
			string_to_log_and_print = f"{sys_quest_num_str} User choice w.r.t. BotError is: {choice_txt}"
	...
	for resp_section_num, response_section in enumerate(response_sections):
		...
		if response_content_list_choice_mode == TypeIn:
			...
			for response_number, response_content in enumerate(response_content_list):
				...
				#text_description_of_systems_turn = f"Bot TypeIn Resp# {response_number}: {response_txt}"
				# 2022-05-31T12:46:40PDT renaming Resp to BotSubQuestion# {response_number} (TypeIn)
				text_description_of_systems_turn = f'BotSubQuestion# {response_number}: SubQuestionTxt: "{response_txt}" AnswerFormat: TypeIn'
