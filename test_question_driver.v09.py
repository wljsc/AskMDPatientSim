...
def question_and_answer_loop(start_conversation_with, log_basedir = None):
	...
	user_reply_script_fileobj = open("tmp_script_01.txt", 'r')
	...
	while True:
		...
		bot_output, user_input_json, question_seq_index = post_next(users_reply, log_dir, question_index_seq, conversation_log_filepath, user_reply_script_fileobj)
		...
		if error_msg:
			...
			user_reply_text = user_input_wrapper("Type In Your Choice Here: ", user_reply_script_fileobj)
		...
		users_reply, question_index_seq = process_system_reply(bot_output, log_dir, conversation_log_filepath, user_reply_script_fileobj)
		
	user_reply_script_fileobj.close()
...
def post_next(payload, log_dir, question_seq_index, conversation_log_filepath, user_reply_script_fileobj):
...
def process_system_reply(json_input, log_dir, conversation_log_filepath, user_reply_script_fileobj):
	"""
...
# ResponseHeader: '{response_header}"""
				...
				user_type_in_response = user_input_wrapper(usr_prompt_txt, user_reply_script_fileobj)
				print()
				print()				
...
"""
			...
			users_choice = user_input_wrapper(usr_prompt_txt, user_reply_script_fileobj)
			print()
			print()			
...
def user_input_wrapper(prompt_text, user_reply_script_fileobj = None):
	...
	if user_reply_script_fileobj is None:
		user_reply_txt = input(prompt_text)
	else:
		current_user_reply_json = user_reply_script_fileobj.readline()
