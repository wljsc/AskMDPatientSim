...
def question_and_answer_loop(start_conversation_with, log_basedir = None):
	...
	while True:
		...
		bot_output, user_input, question_seq_index = post_next(users_reply, log_dir, question_index_seq, conversation_log_filepath)
		...
		if error_msg:
			...
			string_to_log_and_print = f"{sys_quest_num_str} User choice w.r.t. BotError is: {choice_text}"
			...
			if choice_txt == "skip to next question":
				...
				users_reply = skip_to_next_question(user_input)
				continue
		...
		users_reply, question_index_seq = process_system_reply(bot_output, log_dir, conversation_log_filepath)
...
def post_next(payload, log_dir, question_seq_index, conversation_log_filepath):
	...
	print("#### Payload for Ques QuesSeq# {question_seq_index} QuesID# {question_id}")
	...
	sys_quest_num_str = f"QuesSeq# {question_seq_index} QuesID# {question_id}"
	print(f"Bot JSON Response is {sys_quest_num_str}")
	print()
	...
	return(result_json, payload, question_seq_index)

	# The immed above return is the return from @func post_next
...
def process_system_reply(json_input, log_dir, conversation_log_filepath):
	"""
	...
	for resp_section_num, response_section in enumerate(response_sections):
		...
		if response_content_list_choice_mode == TypeIn:
			...
			for nth_choice, response_content in enumerate(response_content_list):
				...
				#text_description_of_systems_turn = f"{sys_quest_num_str} MultiChoiceOption # {nth_choice}: {response_txt}"
				text_description_of_systems_turn = f"Bot MultiChoiceOption # {nth_choice}: {response_txt}"
...
def skip_to_next_question(user_input):
